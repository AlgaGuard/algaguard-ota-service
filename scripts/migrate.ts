...
try {
  await client.query(
    "SELECT pg_advisory_lock(hashtext('algaguard-ota-migrations'))",
  );
  ...
  for (const filename of (await readdir(path.resolve("migrations")))
    ...
    const prior = await client.query(
      ...
      ["algaguard-ota-service", filename],
    );
    if (prior.rows[0]) {
    ...
    try {
      ...
      await client.query(
        ...
        ["algaguard-ota-service", filename, sha],
      );
  ...
  await client.query(
    "SELECT pg_advisory_unlock(hashtext('algaguard-ota-migrations'))",
  );
  client.release();
  await pool.end();
}
